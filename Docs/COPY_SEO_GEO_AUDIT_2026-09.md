...
| `content/blog/bds-modules-down-bcba-alternative.md` | title/meta/body claimed BDS "appears to be down" / DNS outage | rewritten as a study-method comparison; 301 to `/blog/bds-modules-bcba-alternative`; CTAs to `/free-practice/` and `/free-mock-exam/` |
...
- ~~Exam-prep landing pages on the marketing domain.~~ Resolved in this PR. Each remaining exam page now has one job and one study.behaviorschool.com CTA (UTM'd via `behaviorStudyToolsAppHref` with a page-specific `intent`):
  ...
  | `/rbt-study` | ~~RBT exam prep page~~ | 301 to `https://rbtstudy.behaviorschool.com/` (Rob: RBT study is the subdomain, not a marketing page) | — |
  | `/bcba-pacing-planner` | Weekly hours and mock cadence from an exam date | Free practice for the baseline; secondary free mock (was an email form that promised a plan nothing sends) | study schedule, readiness quiz link back from those pages |
  | `/bcba-study-fluency` | Timed practice and mock checkpoints | Free timed mock | school BCBA hub |
  | `/bcba-study-app-school-based-bcbas` | Study-app landing for school BCBA candidates | Free practice (`intent=school_based_bcba`) | — |
  | `/practice`, `/practice/launch`, `/free-study-plan` | ~~on-site practice hub; study-guide PDF hub~~ | 301 to `/free-practice/` (`?mode=full` → `/free-mock-exam/`) and to `/bcba-study-schedule` | — |
...
- ~~`/fba-to-bip`: describes a "free FBA-to-BIP generator" but the wizard is stubbed.~~ Resolved: 301 to `https://plan.behaviorschool.com/`, which already has a Join Waitlist CTA for early access. The stub page is deleted; sitemap and public links updated.
...
**Dead BDS pages removed.** `src/app/compare/behaviorschool-vs-bds/` and `src/app/bds-modules-alternative/` were already 301'd to `/compare/behaviorschool-vs-bds-modules` on `main`, so the earlier "BDS: don't link out, don't restate the catalog" edits had landed on an unreachable page. Both directories are deleted; the live `bds-modules` page now carries that framing. `/passbehavior-alternative` metadata no longer asserts that the competitor's site "appears to be down" (unverified), and its CTAs go through the canonical `/free-practice/` path instead of `study.behaviorschool.com/signup`. The blog post `/blog/bds-modules-down-bcba-alternative` made the same unverified outage claim in the slug, title, and body; it now 301s to `/blog/bds-modules-bcba-alternative`, a study-method comparison that does not claim BDS is down.

**Analytics hydration.** `PrivacyCompliantAnalytics` rendered a consent `<div>` from inside `<head>`, which caused a React hydration error. The component now mounts in `<body>`.

**Exam-prep funnel sweep (Rob, September 11, 2026: "make sure every exam page goes to a reliable sales funnel and we don't have pages scattered around that don't lead to one").** Every exam-prep route on the marketing domain was classified as live or redirected, and each live page was checked for one job and one study.behaviorschool.com CTA.

- *Dead code behind existing 301s, deleted.* Fourteen directories still shipped page files for paths that `next.config.ts` already redirects to the study app, so they never rendered but kept stale copy in the repo where the guards cannot see it (5th Edition framing, "millisecond-precision response time analysis," pass-rate claims, results gated behind sign-in): `bcba-exam-prep` (762 lines plus a stray `page 3.tsx`), `bcba-study-tools`, `bcba-practice-exam`, `free-bcba-mock-exam`, `bcba-test-questions`, `bcba-exam-practice-questions`, `bcba-6th-edition-practice-questions`, `free-bcba-practice-exam`, `bcba-mock-exam-6th-edition`, `bcba-mock-practice-test` (four files), `free-bcba-mock-practice-test` (1,128 lines), `bcba-mock-exam-guide` (1,068 lines), `behavior-study-tools`, `study`. Also removed: the unused `MiniMockSelector` (launched `/quiz/guest`), the `behaviorstudytools.com → /study` host rewrites (that domain is a separate static site, so the rewrite never fired here), the same nine paths from the hardcoded sitemap array (they were already filtered by `legacyRedirectPaths`, but the list read as if they were live at priority 0.9–0.95), the admin sitemap viewer's exam-prep section (now lists the eight live pages), and the SEO-drafts file map entries that pointed at deleted files. `verify-bcba-acquisition-routing.mjs` now scans every public `src/app` file for `/quiz/guest` instead of one deleted client file. No redirect destinations changed.
- *`/practice` and `/practice/launch`, 301'd and deleted.* An on-site "practice hub" that duplicated the study app's own start pages, told candidates to "sign in to unlock results and analytics" (the free diagnostic shows the weakest domain without signup), and launched the full mock via `/quiz/guest?limit=185` rather than `/free-mock-exam/`. No inbound links, not in the sitemap. `/practice → /free-practice/`; `/practice/launch?mode=full → /free-mock-exam/`, otherwise `/free-practice/`.
- *`/free-study-plan`, 301'd to `/bcba-study-schedule` and deleted.* Claimed "40 free practice questions" for both exams (the BCBA diagnostic is 9 questions), sent RBT candidates to `schoolrbt.com` rather than the RBT subdomain, and promised a "7-day study schedule straight to your inbox" that nothing sends (see the lead-magnet note below). No inbound links, not in the sitemap. The two study-guide PDFs it linked (`public/downloads/behavior-school-{bcba,rbt}-study-guide.pdf`) and their SVG covers are now orphaned; the RBT one is written to the "2nd Edition Task List" and states a "~68%" passing score. Left in place because nothing links to them; delete or regenerate before linking them anywhere.
- *`/bcba-pacing-planner` now ends in the funnel.* The planner produced a weekly plan and had no link to Behavior Study Tools at all; its only action was an "Email me the 7-day plan + study checklist" form whose "Sent! Check your inbox" confirmation was untrue. Replaced with a baseline CTA to `/free-practice/` plus a secondary `/free-mock-exam/` link (`intent=bcba_pacing_planner`). The checklist no longer prescribes a "60-question timed mini-mock" the app does not offer. Metadata fixed: house title pattern, canonical added, `siteName` "Behavior School," OG image pointed at a file that exists (`/og-default.png` did not).
- *Copy.* `/bcba-study-fluency` said "BehaviorSchool Study offers" (product name is Behavior Study Tools). `/bcba-study-app-school-based-bcbas` claimed "supervisor reporting," "export progress summaries for supervisors, university cohorts, or employers," a four-label readiness scale, and answered "can a supervisor see my full account?"; none of that is verifiable on the site or in `MARKETING_CONTEXT.md`, so the features now use the approved product language and the FAQ answers the real first question (no account needed for the 9-question check or the 185-question mock). The same claim sat in the shared `BcbaSeoLanding` trust strip ("Supervisor report export," "Readiness score") and in the BDS compare page's meta description, intro, and "Sharing progress" row ("built to be shared with a supervisor"); all replaced with the approved language. If the supervisor export is a real, shipped feature, it can be restored with a source.
- *Already in shape, no change:* `/bcba-readiness-quiz`, `/bcba-exam-weak-areas`, `/bcba-study-schedule`, `/failed-bcba-exam-help`, `/ebook/bcba-exam-guide`, the four `/compare/*` pages, `/passbehavior-alternative` (all fixed earlier in this PR; see §2.1 and §2.5).
- *Lead-magnet note (not changed, needs a product decision).* `/api/lead-magnet` upserts a CRM contact and starts The Weekly Research Brief double opt-in; it never emails the resource the form promised. Remaining callers are `/school-bcba/first-90-days` and `/bcba-burnout-quiz/playbook` (Transformation-funnel pages, where the newsletter subscription is at least the intended outcome). Any future exam-prep lead form should either deliver the file on the confirmation screen (as the two ebook gates do via `/api/crm`) or send it through a dedicated exam-prep sequence, per the AGENTS.md rule keeping exam-prep email separate from the newsletter.
...
9. ~~**`/fba-to-bip` stub.**~~ 301 to `https://plan.behaviorschool.com/` (Rob: the destination page already has the early-access waitlist CTA). Stub page deleted; `/fba-bip-plan-writer` now points at the same URL instead of chaining. Public marketing and blog CTAs no longer promise a live free generator.
10. ~~**Salary data sourcing.**~~ Decided: do not invent a named district-by-district dataset. Keep the directional snapshot, show the last-reviewed date from `public/data/salary-benchmarks.json` (currently January 15, 2025), and refresh **quarterly if `/school-bcba/salary-by-state` is getting search traffic**. The GitHub Action `salary-data-refresh.yml` now runs on a quarterly cron and only proceeds when the Actions variable `SALARY_PAGE_HAS_TRAFFIC` is `true` (or on manual `workflow_dispatch`). Page copy and JSON-LD no longer claim a current-year scrape.
11. ~~Stray duplicate files.~~ Deleted all tracked macOS `* 2` copies (19 files). None were imported.
12. ~~The BCBA Exam Survival Guide PDF is written to the 5th Edition Task List.~~ Rebuilt on the 6th Edition. `public/ebooks/bcba-exam-guide-2026.pdf` is now generated from `scripts/generate-bcba-exam-guide-pdf.tsx` (`pnpm pdf:bcba-exam-guide`, `@react-pdf/renderer`, 13 pages) so it can be regenerated when the BACB revises the outline. Domain names, task counts (104), scored-question counts (175, plus 10 unscored pilot = 185), and percentages are copied from the BACB *BCBA Test Content Outline (6th ed.)*, updated 09/2024, and the script asserts the totals. Content: what the exam is and two "edition tells" for spotting stale materials; the nine-domain table plus a questions-per-task read and a factual "what changed from the 5th Edition" list (Task List → TCO, 92 → 104 tasks, A and E renamed, E.9–E.11 cultural humility/responsiveness/bias, weight shift toward F/H/I); a 12-week schedule that assigns domains to weeks with two full timed mocks and the four approved readiness signals; concept chapter (reinforcement/punishment, functions and assessment methods, ethics sequence, measurement, supervision); pacing at 78 s/question; mistakes with fixes; official BACB documents and two textbooks; a 30-day checklist; and a single CTA to `study.behaviorschool.com/free-practice/` with UTMs. Removed from the old file: the "64% first-time pass rate" statistic, the "thousands pass every year" line, "your first instinct is usually correct," the competitor question banks (Pass the Big ABA Exam, ABA Exam Review), and the closing "BehaviorSchool Pro / AI-powered / free 14-day trial" pitch (Pro is the FBA/BIP workspace, not exam prep). The retired 11-page duplicate `public/downloads/bcba-exam-survival-guide-2026.pdf` was deleted (nothing linked to it). Landing-page copy, chapter cards, and meta description now say 6th Edition without hedging. Separately, the email gate had never worked: it posted `firstName` but `/api/crm` requires `name`, so every submission got a 400 and nobody received the file. Fixed here. `/ebook/school-bcba-starter-kit` had the same bug and is now fixed the same way (`name: firstName`); `/api/crm` also accepts `firstName` as a fallback so other gates cannot 400 the same way.
13. ~~**`/rbt-study` is not in `sitemap.ts`.**~~ Decided: RBT study is the subdomain, not a marketing page. `/rbt-study` now 301s to `https://rbtstudy.behaviorschool.com/`; the stub page is deleted and the path is in `legacyRedirectPaths` / `verify-canonical-links.mjs`. Not added to the sitemap.
14. ~~All five `/videos/*` pages are in the sitemap with placeholder YouTube URLs.~~ Fixed. `src/lib/videos.ts` now owns video loading and `isPublishedVideo()` (rejects empty or `PLACEHOLDER` `videoUrl`s). Video detail pages are only generated for published videos and return 404 otherwise; the library page and sitemap list published videos only; `/videos` itself is `noindex, nofollow` and out of the sitemap until at least one video is published (still subject to the admin indexing toggles). With zero published videos, the library page no longer shows the FAQ block or `FAQPage` JSON-LD that claimed "we offer free BCBA exam prep videos… all videos are available," and the hero and empty state say recording is in progress. Both video pages' "Start Free Trial" CTAs pointed at `/signup`, the Transformation System application; they now go to the free 6th Edition practice set with UTMs. Adding the real YouTube URLs in the admin videos tool publishes everything automatically.
15. ~~`/calaba40` claims "10,000+ AI Questions."~~ Replaced. The six feature cards on the (noindex) coupon page now use approved product language: adaptive practice by missed domain, a rationale on every answer, full mocks (185 questions, four hours, results by domain), web app on any device, 6th Edition aligned, readiness signals. The unverified count, the "AI Tutor" card, and the "AI-Powered" H1 and meta description lead are gone. The footer's "valid through March 31, 2026" expiry line was removed at Rob's request; the page and code stay live.
16. **Two orphan PDFs still mention the 5th Edition Task List.** Resolved at Rob's request: both were regenerated on the 6th Edition Test Content Outline with `pnpm pdf:cited-lead-magnets` (`scripts/generate-all-cited-pdfs.tsx`, output path is now repo-relative instead of a hardcoded local volume). `12-week-bcba-study-schedule.pdf` was rewritten to match Chapter 2 of the exam guide: one domain per week using the 6th Edition domain names and task/question counts, a week-4 baseline set, two full mocks in weeks 10 and 11, the four readiness signals, and the BACB 6th Edition reference. `10-bcba-practice-questions.pdf` had its edition line updated and each question tagged with its 6th Edition domain; the questions themselves are edition-neutral. Both PDFs are still unlinked from any page. Open: the questions PDF is titled "10 BCBA Practice Questions" but contains four; write six more or retitle it before linking it anywhere.
