...
export function AudioNarration({ slug, title }: AudioNarrationProps) {
  ...
  // Check for pre-generated audio first, then fall back to API generation
  useEffect(() => {
    ...
    fetch(preGeneratedUrl, { method: 'HEAD' })
      .then(res => {
        ...
        // If not found, we'll generate on-demand when user clicks play
      })
      .catch(() => {
  ...
  const generateAudio = async () => {
    ...
    try {
      const response = await fetch(`/api/blog/audio/${slug}`, {
        method: 'POST',
      });
  ...
  return (
    <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl p-4 my-6">
