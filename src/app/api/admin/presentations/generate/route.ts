...
export async function POST(request: NextRequest) {
  try {
    ...
    const { topic, slideCount = 10, template = 'modern', tone = 'professional', language = 'English', model = 'gemini-1.5-pro', provider = 'google', apiKey, exportAs = 'pptx', ollamaEndpoint, slides } = body;
    ...
    if (exportAs === 'pdf') {
      ...
      try {
        const html = await buildHtmlFromSlides(topic, slideContent as any, template);
    ...
    const pptBuffer = await buildPptxFromSlides(topic, slideContent, template);
    await persistPresentation({
      buffer: pptBuffer,
      topic,
      slideCount,
...
async function buildPptxFromSlides(
  ...
  titleSlide.addText(topic, {
    ...
    fontSize: 44, bold: true, color: templateConfig.titleColor, align: 'center',
  });
  titleSlide.addText('Generated by Behavior School AI', {
    x: 0.5, y: 5.0, w: '90%', h: 0.5,
    fontSize: 16, color: templateConfig.subtitleColor, align: 'center',
  });
  ...
  for (const slide of slideContent) {
    ...
    // Title
    contentSlide.addText(slide.title, { x: 0.5, y: 0.5, w: '90%', h: 0.75, fontSize: 32, bold: true, color: templateConfig.titleColor });

    const bullets = slide.content.map((point) => ({ text: point, options: { bullet: true } }));

    if (resolveLayout === 'text') {
      contentSlide.addText(bullets, { x: 0.5, y: 1.5, w: '90%', h: 4.0, fontSize: 18, color: templateConfig.textColor, bullet: { type: 'bullet' }, valign: 'top' });
      ...
      contentSlide.addText(bullets, { x: 0.5, y: 1.5, w: 5.0, h: 4.0, fontSize: 18, color: templateConfig.textColor, bullet: { type: 'bullet' }, valign: 'top' });
      ...
      contentSlide.addText(bullets, { x: 5.3, y: 1.5, w: 4.5, h: 4.0, fontSize: 18, color: templateConfig.textColor, bullet: { type: 'bullet' }, valign: 'top' });
    } else if (resolveLayout === 'two-column') {
      const mid = Math.ceil(bullets.length / 2);
      const left = bullets.slice(0, mid);
      const right = bullets.slice(mid);
      contentSlide.addText(left, { x: 0.5, y: 1.5, w: 4.5, h: 4.0, fontSize: 18, color: templateConfig.textColor, bullet: { type: 'bullet' }, valign: 'top' });
      contentSlide.addText(right, { x: 5.3, y: 1.5, w: 4.5, h: 4.0, fontSize: 18, color: templateConfig.textColor, bullet: { type: 'bullet' }, valign: 'top' });
    } else if (resolveLayout === 'quote') {
      const quote = slide.content[0] || '';
      contentSlide.addText(`“${quote}”`, { x: 0.7, y: 1.8, w: 8.6, h: 3.0, fontSize: 28, italic: true, color: templateConfig.textColor, align: 'center' });
      ...
      contentSlide.addText(bullets, { x: textX, y: 1.5, w: 4.8, h: 4.0, fontSize: 18, color: templateConfig.textColor, bullet: { type: 'bullet' }, valign: 'top' });
    }
      ...
      if (data) {
        ...
        for (let i=0;i<catCount;i++) {
          ...
          data.series.forEach((s, si) => {
            ...
            if (!data.stacked) {
              try { page.drawText(formatValue(v, slide.chart?.yFormat), { x, y: yBar + h + 2, size: 8, font: helvetica, color: hexToRgb(templateConfig.textColor) }); } catch {}
            }
...
async function buildHtmlFromSlides(topic: string, slides: SlideInput[], template: string): Promise<string> {
      ...
      if (s.chart && s.chart.categories?.length && s.chart.series?.length) {
        ...
        if (s.chart.type === 'bar') {
          ...
          const bars = cat.map((label, ci) => {
            const group = s.chart!.stacked
              ? `<div class=\"stack\">${ser.map((ss, si) => {
                  const h = max>0 ? Math.max(4, Math.round((ss.values[ci]||0)/max*100)) : 0;
                  return `<div class=\"seg\" style=\"background:${colorsArr[si]};height:${h}%\"></div>`;
                }).join('')}</div>`
              : `<div class=\"cluster\">${ser.map((ss, si) => {
                  const h = max>0 ? Math.max(4, Math.round((ss.values[ci]||0)/max*100)) : 0;
                  return `<div class=\"bar\" style=\"background:${colorsArr[si]};height:${h}%\"></div>`;
                }).join('')}</div>`;
            return `<div class=\"cat\"><div class=\"col\">${group}</div><span class=\"xlabel\">${esc(label)}</span></div>`;
          }).join('');
          const ticks = yTicks.map(v=> `<div class=\"tick\"><span>${esc(formatNumber(v))}</span></div>`).join('');
          return `<div class=\"chart ygrid\"><div class=\"yticks\">${ticks}</div><div class=\"groups\">${bars}</div>${legend}</div>`;
        }
        ...
        if (s.chart.type === 'pie' || s.chart.type === 'doughnut') {
          ...
          acc = 0;
          const labels = series.values.map((v, i) => {
          ...
          const labels = cat.map((c, i)=> `<div><span style=\"background:${colorsArr[i % colorsArr.length]}\"></span>${esc(c)}</div>`).join('');
          return `<div class=\"chart pie\"><svg width=\"100%\" height=\"360\" viewBox=\"0 0 520 380\">${slices}${hole}${labels}</svg><div class=\"legend\">${labels}</div></div>`;
        }
      }
  ...
  <style>
    ...
    *{ box-sizing:border-box; }
    body{ margin:0; background:#f8fafc; font-family: system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, Noto Sans, 'Apple Color Emoji','Segoe UI Emoji';}
    .slide{ width:1280px; height:720px; padding:40px; background:#${colors.backgroundColor}; position:relative; page-break-after: always; }
    .title{ margin:0 0 20px; color:#${colors.titleColor}; font-size:38px; font-weight:800; }
    ...
    .col{ width:100%; height:100%; display:flex; align-items:flex-end; gap:6px; }
