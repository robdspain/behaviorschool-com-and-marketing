...
async function buildPptxFromSlides(
  ...
  titleSlide.addText(topic, {
    ...
    fontSize: 44, bold: true, color: templateConfig.titleColor, align: 'center', fontFace: 'Arial',
  });
  titleSlide.addText('Generated by Behavior School AI', {
    x: 0.5, y: 5.0, w: '90%', h: 0.5,
    fontSize: 16, color: templateConfig.subtitleColor, align: 'center', fontFace: 'Arial',
  });
  ...
  for (const slide of slideContent) {
    ...
    // Title
    contentSlide.addText(slide.title, { x: 0.5, y: 0.5, w: '90%', h: 0.75, fontSize: 32, bold: true, color: templateConfig.titleColor, fontFace: 'Arial' });

    const bullets = slide.content.map((point) => ({ text: point, options: { bullet: true } }));

    if (resolveLayout === 'text') {
      contentSlide.addText(bullets, { x: 0.5, y: 1.5, w: '90%', h: 4.0, fontSize: 18, color: templateConfig.textColor, bullet: { type: 'bullet' }, valign: 'top', fontFace: 'Arial' });
      ...
      contentSlide.addText(bullets, { x: 0.5, y: 1.5, w: 5.0, h: 4.0, fontSize: 18, color: templateConfig.textColor, bullet: { type: 'bullet' }, valign: 'top', fontFace: 'Arial' });
      ...
      contentSlide.addText(bullets, { x: 5.3, y: 1.5, w: 4.5, h: 4.0, fontSize: 18, color: templateConfig.textColor, bullet: { type: 'bullet' }, valign: 'top', fontFace: 'Arial' });
...
async function buildHtmlFromSlides(topic: string, slides: SlideInput[], template: string): Promise<string> {
        ...
        if (s.chart.type === 'bar') {
          ...
          const bars = cat.map((label, ci) => {
            if (s.chart!.stacked) {
              ...
              const labelEl = `<span class=\"val\" style=\"bottom: calc(${Math.min(accPct,100)}% + 2px)\">${esc(formatValue(total, s.chart?.yFormat))}</span>`;
              ...
              const cols = ser.map((ss, si) => {
                const v = ss.values[ci]||0; const h = max>0 ? Math.max(4, Math.round((v/max)*100)) : 0;
                const lbl = `<span class=\"val\" style=\"bottom: calc(${h}% + 2px)\">${esc(formatValue(v, s.chart?.yFormat))}</span>`;
  ...
  <style>
    ...
    *{ box-sizing:border-box; }
    ...
    .val{ position:absolute; font-size:10px; color:#${colors.textColor}; transform:translateY(0); left:4px; }
