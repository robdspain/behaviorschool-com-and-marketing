...
// Sanitize text for pdf-lib StandardFonts (only supports Latin-1 / chars <= 255)
function sanitizePdfText(text: string): string {
  return text
    ...
    .replace(/[^\x00-\xFF]/g, '');            // strip anything else > 255
}
...
async function buildPdfFromSlides(
  ...
  page.drawText(sanitizePdfText(topic), { x: 50, y: 540, size: 28, font: helveticaBold, color: hexToRgb(templateConfig.titleColor) });
  page.drawText(sanitizePdfText('Generated by Behavior School AI'), { x: 50, y: 510, size: 12, font: helvetica, color: hexToRgb(templateConfig.subtitleColor) });

  // Content pages
  for (const slide of slideContent) {
    page = pdfDoc.addPage([612, 792]);
    page.drawText(sanitizePdfText(slide.title), { x: 50, y: 720, size: 20, font: helveticaBold, color: hexToRgb(templateConfig.titleColor) });
    const layout = slide.layout || 'auto';
    ...
    if (resolveLayout === 'text') {
      ...
      const quote = slide.content[0] || '';
      page.drawText(sanitizePdfText(`"${quote}"`), { x: 60, y: 620, size: 20, font: helveticaBold, color: hexToRgb(templateConfig.textColor), maxWidth: 500 });
      y = 580;
    } else if (resolveLayout === 'title-only') {
      // nothing
      y = 0;
    ...
    if (resolveLayout === 'two-column') {
      ...
      for (const line of left) { page.drawText(sanitizePdfText(`• ${line}`), { x: 60, y: yL, size: 12, font: helvetica, color: hexToRgb(templateConfig.textColor), maxWidth: 240 }); yL -= 20; }
      for (const line of right) { page.drawText(sanitizePdfText(`• ${line}`), { x: 340, y: yR, size: 12, font: helvetica, color: hexToRgb(templateConfig.textColor), maxWidth: 240 }); yR -= 20; }
    } else if (resolveLayout === 'metrics-3') {
      ...
      if (data) {
        ...
        // axis labels
        if (slide.chart?.xLabel) try { page.drawText(sanitizePdfText(slide.chart.xLabel), { x: chartX + chartW/2 - 20, y: chartY - 26, size: 10, font: helvetica, color: hexToRgb(templateConfig.textColor) }); } catch {}
        if (slide.chart?.yLabel) try { page.drawText(sanitizePdfText(slide.chart.yLabel), { x: chartX + 4, y: chartY + chartH/2, size: 10, font: helvetica, color: hexToRgb(templateConfig.textColor) }); } catch {}
      } else {
      ...
      }
      {
        ...
        nb2.items.forEach((line, idx) => {
          ...
          page.drawText(sanitizePdfText(`${b} ${line}`), { x: textX0 + 10, y, size: 12, font: helvetica, color: hexToRgb(templateConfig.textColor), maxWidth: 240 });
          y -= 20;
      ...
      if (headers.length) {
        for (let ci = 0; ci < headers.length; ci++) {
          ...
          page.drawText(sanitizePdfText(String(headers[ci])), { x: startX + ci*colW + 6, y: startY + 6, size: 12, font: helveticaBold, color: hexToRgb(templateConfig.titleColor), maxWidth: colW - 12 });
        }
      }
      // Rows
      ...
      rows.forEach((r) => {
        for (let ci = 0; ci < colCount; ci++) {
          ...
          page.drawText(sanitizePdfText(cell), { x: startX + ci*colW + 6, y: yRow + 6, size: 10, font: helvetica, color: hexToRgb(templateConfig.textColor), maxWidth: colW - 12 });
        }
        yRow -= rowH;
      });
    } else if (y > 0) {
      ...
      nb3.items.forEach((line, idx) => {
        ...
        page.drawText(sanitizePdfText(`${b} ${line}`), { x: textX + 10, y, size: 12, font: helvetica, color: hexToRgb(templateConfig.textColor), maxWidth: textW });
        y -= 20;
        if (y < 60) { y = 690; page = pdfDoc.addPage([612, 792]); }
      });
    }
