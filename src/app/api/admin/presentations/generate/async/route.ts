...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Mark running
    await getSupabase().from('presentations_ai_jobs').update({ status: 'running', progress: 10, updated_at: new Date().toISOString() }).eq('id', job.id);
    ...
    if (!res.ok) {
      ...
      await getSupabase().from('presentations_ai_jobs').update({ status: 'error', error: errText, progress: 100, updated_at: new Date().toISOString() }).eq('id', job.id);
    ...
    await getSupabase().from('presentations_ai_jobs').update({ status: 'completed', progress: 100, updated_at: new Date().toISOString() }).eq('id', job.id);
